...
var CS=[['沐浴','冠带','临官','帝旺','衰','病','死','墓','绝','胎','养','长生'],['病','衰','帝旺','临官','冠带','沐浴','长生','养','胎','绝','墓','死'],['胎','养','长生','沐浴','冠带','临官','帝旺','衰','病','死','墓','绝'],['绝','墓','死','病','衰','帝旺','临官','冠带','沐浴','长生','养','胎'],['胎','养','长生','沐浴','冠带','临官','帝旺','衰','病','死','墓','绝'],['绝','墓','死','病','衰','帝旺','临官','冠带','沐浴','长生','养','胎'],['死','墓','绝','胎','养','长生','沐浴','冠带','临官','帝旺','衰','病'],['长生','养','胎','绝','墓','死','病','衰','帝旺','临官','冠带','沐浴'],['帝旺','衰','病','死','墓','绝','胎','养','长生','沐浴','冠带','临官'],['临官','冠带','沐浴','长生','养','胎','绝','墓','死','病','衰','帝旺']];
...
function openAI(){var r;try{r=BaZi.calculateBaZi({year:+$("y").value,month:+$("m").value,day:+$("d").value,hour:+$("h").value,minute:+$("mi").value,gender:document.querySelector("input[name=g]:checked").value})}catch(e){}var h='<div class=fc><div class=ft>\uD83E\uDD16 AI \u547d\u7406\u5206\u6790</div>';h+='<textarea id=aiPrompt style="width:100%;min-height:100px;background:rgba(255,255,255,.03);border:1px solid rgba(180,140,100,.12);border-radius:6px;color:#e0d8d0;padding:8px;font-size:12px;outline:none;resize:vertical">';if(r){h+='\u751f\u5c5e\u5de5\u5177\uff1a\u516b\u5b57\u6392\u76d8AI\u5206\u6790\n\u6027\u522b\uff1a'+(r.input.gender==='male'?'\u7537\u547d':'\u5973\u547d')+'\n\u56db\u67f1\uff1a'+r.pillars.year.ganName+r.pillars.year.zhiName+' '+r.pillars.month.ganName+r.pillars.month.zhiName+' '+r.pillars.day.ganName+r.pillars.day.zhiName+' '+r.pillars.hour.ganName+r.pillars.hour.zhiName;if(r.deityAnalysis){h+='\n\u7528\u795e\uff1a'+r.deityAnalysis.usefulDeities.join('\u3001')+'\n\u5fcc\u795e\uff1a'+r.deityAnalysis.harmfulDeities.join('\u3001')}}\n\n请作为精通国学易经数术的资深命理分析师，核心擅长主流子平派格局理论与新派命理技法。基于固定命盘信息精准分析五行生克、十神组合等关系得出喜用忌神，围绕事业、财运、婚恋、子女、六亲等维度展开深度解析。关键事件须给出发生时间范围、喜忌属性、事件对命主的影响程度。输出格式：1.命盘技法解读 2.命盘事项解读 3.大运流年走势解读 4.核心建议与风险规避。\n\n前置声明：本分析仅基于传统八字命理理论逻辑，不涉及科学实证结论，仅供文化娱乐参考，非专业决策依据，具体发展需结合个人努力与客观环境。'h+='</textarea>';h+='<div style="display:flex;gap:6px;margin:6px 0;align-items:center"><button class=btn style="width:auto;padding:8px 20px" onclick=sendAI()>\u53d1\u9001</button>';h+='<span style="font-size:10px;color:#786a5e;cursor:pointer" onclick="var e=document.getElementById(\'aiSettings\');e.style.display=e.style.display===\'none\'?\'block\':\'none\'">\u26a1 \u8bbe\u7f6e</span>';h+='</div>';h+='<div id=aiSettings style="display:none">';h+='<div style="display:flex;gap:6px;margin:4px 0;align-items:center"><label style="font-size:10px;color:#786a5e;width:60px">API Key</label><input type=password id=aiKey style="flex:1;padding:5px;border:1px solid rgba(180,140,100,.12);border-radius:4px;background:rgba(255,255,255,.03);color:#e0d8d0;font-size:11px;outline:none"></div>';h+='<div style="display:flex;gap:6px;margin:4px 0;align-items:center"><label style="font-size:10px;color:#786a5e;width:60px">\u6a21\u578b</label><input type=text id=aiModel value="deepseek-chat" style="flex:1;padding:5px;border:1px solid rgba(180,140,100,.12);border-radius:4px;background:rgba(255,255,255,.03);color:#e0d8d0;font-size:11px;outline:none"></div>';h+='<div style="display:flex;gap:6px;margin:4px 0;align-items:center"><label style="font-size:10px;color:#786a5e;width:60px">\u7f51\u5173</label><input type=text id=aiUrl value="https://api.deepseek.com/v1/chat/completions" style="flex:1;padding:5px;border:1px solid rgba(180,140,100,.12);border-radius:4px;background:rgba(255,255,255,.03);color:#e0d8d0;font-size:11px;outline:none"></div>';h+='</div>';h+='<div id=aiResponse style="font-size:12px;color:#9a8a7a;line-height:1.6;white-space:pre-wrap;margin-top:6px;display:none;max-height:400px;overflow-y:auto;background:rgba(255,255,255,.02);border-radius:6px;padding:10px"></div>';h+='</div>';document.getElementById('aiSection').innerHTML=h;document.getElementById('aiSection').style.display='block'}function closeAI(){document.getElementById("aiSection").style.display="none"}
function sendAI(){var prompt=document.getElementById("aiPrompt").value;var key=document.getElementById("aiKey").value;var url=document.getElementById("aiUrl").value;var resp=document.getElementById("aiResponse");resp.style.display="block";resp.textContent="正在请求AI分析……";fetch(url,{method:"POST",headers:{"Content-Type":"application/json","Authorization":"Bearer "+key},body:JSON.stringify({model:url.includes("deepseek")?"deepseek-chat":"gpt-4o-mini",messages:[{role:"system",content:"你是一名精通国学易经数术的资深命理分析师，请根据用户提供的命盘信息进行深度分析。"},{role:"user",content:prompt}]})}).then(function(res){return res.json()}).then(function(data){var text=data.choices?.[0]?.message?.content||JSON.stringify(data);resp.textContent=text}).catch(function(err){resp.textContent="请求失败："+err.message})}
function genAIPrompt(){
  try{
    var fr=BaZi.calculateBaZi({year:+$("y").value,month:+$("m").value,day:+$("d").value,hour:+$("h").value,minute:+$("mi").value,gender:document.querySelector("input[name=g]:checked").value});
    var p=fr.pillars,ps=[p.year,p.month,p.day,p.hour],ld=fr.lunarDate||{},wy=fr.wuXingAnalysis,da=fr.deityAnalysis;
    var wxc=["木","火","土","金","水"];
    var lines=[];
    lines.push("生属工具：八字排盘AI分析");
    lines.push("");
    lines.push("## 命盘基础信息");
    lines.push("性别："+(fr.input.gender==="male"?"男命":"女命"));
    lines.push("出生："+fr.input.year+"年"+fr.input.month+"月"+fr.input.day+"日 "+fr.input.hour+":"+("0"+fr.input.minute).slice(-2));
    lines.push("农历："+(ld.lunarYearGz||"")+"年 "+(ld.monthName||"")+(ld.dayName||""));
    lines.push("四柱："+ps.map(function(p){return p.ganName+p.zhiName}).join(" "));
    lines.push("日主："+fr.dayMaster.name+fr.dayMaster.element+"("+(fr.dayMaster.yin?"阴":"阳")+")");
    lines.push("日支："+p.day.zhiName+fr.pillars.day.zhiName);
    lines.push("月令："+p.month.zhiName+"月");
    if(wy){lines.push("旺相："+wxc.map(function(e){return e+":"+wy.wangShuai[e]}).join(" "));}
    if(da){
      lines.push("格局类型："+(fr.tenGods.yearGan==="七杀"?"七杀格":"正官格"));
      lines.push("衰嫁类型："+da.strengthLevel);
      lines.push("调候用神："+da.tiaoHou.join("、"));
      lines.push("用神："+da.usefulDeities.join("、"));
      lines.push("忌神："+da.harmfulDeities.join("、"));
      lines.push("策略："+da.strategy);
    }
    lines.push("大运："+fr.greatFortunes.map(function(ft){return ft.ganName+ft.zhiName+"("+ft.startAge+"-"+ft.endAge+"岁)"}).join(" "));
    lines.push("");
    lines.push("请作为精通国学易经数术的资深命理分析师，围绕事业、财运、婚恋、子女、六亲等维度展开深度解析。");
    lines.push("（以上分析仅作为文化娱乐参考，不构成任何决策依据）");
    var text=lines.join("\n");
    if(navigator.clipboard){navigator.clipboard.writeText(text).then(function(){alert("已复制AI分析指令到剪贴板\uff0c请粘贴到AI对话框中。")}).catch(function(){alert("复制失败，请手动复制")})}else{alert(text)}
...
var mode='bazi';function toggleMode(){var e=$('baziForm'),f=$('lunarForm'),m=$('modeBtn');if(mode==='bazi'){mode='lunar';e.style.display='none';f.style.display='block';m.textContent='公历→兮历'}else{mode='bazi';e.style.display='flex';f.style.display='none';m.textContent='兮历→公历'}}
function doLunarConv(){try{var r=BaZi.lunarToSolar(+$('ly').value,+$('lm').value,+$('ld').value,+$('ll').value===1);if(r)$('lunarResult').textContent='对并日：'+r.year+'年'+r.month+'月'+r.day+'日';else $('lunarResult').textContent='输数'}catch(e){$('lunarResult').textContent='ERR'}}
function calc(){
...
function toggleInfo(){var el=document.querySelector(".it");if(el){var s=el.style.display;if(s==="none"){el.style.display="";start()}else{el.style.display="none"}}}
function mainRender(r){
  ...
  h+='<div class=ll>'+__('农历')+'：'+r.input.year+('年')+' '+ld.month+'月'+ld.day+'日'+' '+r.input.hour+__('时')+' '+tg+'</div>';
  ...
  h+='<div class=ia><button onclick=window.print()>\uD83D\uDCC4</button><button id=toggleBtn onclick=toggleInfo()>\uD83D\uDC41</button><button onclick=openAI()>\uD83E\uDD16</button></div></div>';

  var mainStars=[r.tenGods.yearGan,r.tenGods.monthGan,gc2,r.tenGods.hourGan];
  var kongVals=ps.map(function(p){for(var sx=0;sx<60;sx++){if(sx%10===p.gan&&sx%12===p.zhi)return KW.substr(Math.floor(sx/10)*2,2)}return ''});
  ...
  h+='</tr><tr><td>'+__('星运')+'</td>';for(var dm=r.pillars.day.gan,i=0;i<4;i++){h+='<td style=color:#887a6e>'+(CS[dm]?.[ps[i].zhi]||'')+'</td>';}
  ...
  h+='</tr><tr><td>'+__('神煞')+'</td>';for(var i=0;i<4;i++){var ss2=r.shenShaDetail[i].map(function(s){return'<em>'+s.name+'</em>'});h+='<td class=ss>'+(ss2.join('')||'')+'</td>';}
  ...
  h+='<div class=fc><div class=ft>原局干支关系</div><div style=font-size:11px;color:#9a8a7a;line-height:1.7>'
  // 天干关系
  var ganTypes={};var zhiTypes={};
  for(var k=0;k<r.interactions.length;k++){
    var it=r.interactions[k];
    if(it.type.includes('天干')){if(!ganTypes[it.type])ganTypes[it.type]=[];ganTypes[it.type].push(it.description);}
    else{if(!zhiTypes[it.type])zhiTypes[it.type]=[];zhiTypes[it.type].push(it.description);}
  }
  var typeOrder=['天干五合','天干五冲','三会','三合','半合','六合','六冲','六害','六破','三刑','自刑','禄'];
  for(var t=0;t<typeOrder.length;t++){
    var tp=typeOrder[t];
    if(ganTypes[tp]){h+='<span style=color:#c4a265;margin-right:4px>天干'+(tp.replace('天干',''))+'：</span>'+ganTypes[tp].join('、')+'<br>';}
    if(zhiTypes[tp]){h+='<span style=color:#c4a265;margin-right:4px>'+(tp=='三会'?'三会局':tp=='三合'?'三合局':tp=='半合'?'半合':tp=='六合'?'六合':tp=='六冲'?'六冲':tp=='六害'?'六害':tp=='六破'?'六破':tp=='三刑'?'三刑':tp=='自刑'?'自刑':tp)+'：</span>'+zhiTypes[tp].join('、')+'<br>';}
  }
  if(Object.keys(ganTypes).length===0&&Object.keys(zhiTypes).length===0)h+='无特殊';
  h+='</div></div>';
  ...
  h+='<div style="display:flex;padding:2px 0;color:'+ai.color+';align-items:center;white-space:nowrap;overflow:hidden"><span style="width:50px;text-align:center;flex-shrink:0">'+ai.year+'</span><span style="width:50px;text-align:center;flex-shrink:0">'+ai.ganZhi+'</span><span style="flex:1;font-size:9px;overflow:hidden;text-overflow:ellipsis">';
  if(ai.zhiInteraction!=='无特殊')h+=__('地支')+ai.zhiInteraction;
  if(ai.ganHe!=='无')h+=(ai.zhiInteraction!=='无特殊'?' | ':'')+ai.ganHe;
  h+='</span><span style="width:70px;text-align:right;font-size:9px;flex-shrink:0">'+ai.fortuneEffect+'</span></div>';}
